...
public class OpenQuestionAssignment extends Assignment {
    ...
    private Integer maxLength; // Maksymalna długość odpowiedzi w znakach

    @Column(name = "min_length")
    private Integer minLength; // Minimalna długość odpowiedzi w znakach

    @Column(name = "sample_answer", length = 5000)
    private String sampleAnswer; // Przykładowa odpowiedź dla nauczyciela

    @Column(name = "grading_rubric", length = 2000)
    private String gradingRubric; // Kryteria oceniania

    @Column(name = "allow_html")
    @Builder.Default
    private Boolean allowHtml = false; // Czy pozwalać na formatowanie HTML

    @Column(name = "case_sensitive")
    @Builder.Default
    private Boolean caseSensitive = false; // Czy rozróżniać wielkość liter (dla prostych porównań)
